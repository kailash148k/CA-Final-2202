...
# Ensures folders exist on the Streamlit server to prevent OSErrors
...
# --- 3. LIBRARIES ---
TRADING_PL_HEADS = [
    "Sales", "Opening Stock", "Purchases", "Wages", "Direct Expenses", "Salaries", 
    "Office Rent", "Audit Fees", "Depreciation", "Interest Received", "Commission Received"
]

BALANCE_SHEET_HEADS = [
    "Proprietor's Capital", "Secured Loans", "Unsecured Loans", "Sundry Creditors", 
    "Duties & Taxes", "Sundry Debtors", "Cash-in-Hand", "Balance with Banks", "Security Deposits"
]

# --- 4. SIDEBAR: KYC, ITR & REFERENCES ---
with st.sidebar:
...
# --- 5. RUNNING LOANS: INTEREST SYNC & PRE-CLOSURE ---
...
for idx, loan in enumerate(st.session_state.detailed_loans):
    with st.container(border=True):
        c1, c2, c3, c4, c5, c6 = st.columns([2, 1, 1, 1, 1, 1.2])
        ...
        # Pre-closure Date Box
        has_closed = c6.checkbox("Closed Early?", key=f"closed_chk_{idx}")
        pre_closure_date = None
        if has_closed:
            pre_closure_date = c6.date_input("Closure Date", value=date.today(), key=f"pcd_{idx}")
        ...
        add_int = o2.checkbox("Add Interest to Income?", value=True, key=f"ai_{idx}")
        
        # Eligibility Logic: Check if loan is currently active
        is_still_active = not (pre_closure_date and pre_closure_date <= date.today())

        if is_ob and is_still_active: 
            total_active_emi += emi
        if add_int: 
            total_interest_to_addback += est_annual_int
            
        status = "✅ Active" if is_still_active else "❌ Closed/Settled"
        st.caption(f"Status: {status} | EMI: ₹{emi:,.0f} | Est. Yearly Interest: ₹{est_annual_int:,.0f}")

# --- 6. DATA TEMPLATES & TABS ---
...
with t_eligibility:
    ...
    with col_e1:
        ...
        int_sync = st.number_input("Interest (Synced from Loans)", value=float(total_interest_to_addback))
        cash_p = npbt + dep_val + int_sync
        st.metric("Adjusted Annual Cash Profit", f"₹{cash_p:,.2f}")

    with col_e2:
        st.metric("Active Monthly EMI Obligations", f"₹{total_active_emi:,.2f}")
        foir = st.slider("FOIR %", 10, 100, 60)
    
    eligible_emi = max(0.0, ((cash_p/12) * (foir/100)) - total_active_emi)
    st.divider()
    if eligible_emi > 0:
        st.success(f"### Eligible Additional Monthly EMI: ₹{eligible_emi:,.2f}")
